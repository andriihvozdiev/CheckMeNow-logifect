...
class SwipeablePanel extends Component<SwipeablePanelProps, SwipeablePanelState> {
    ...
    this._panResponder = PanResponder.create({
      ...
      onPanResponderRelease: (evt, gestureState) => {
        ...
        if (gestureState.dy === 0) {
          ...
          else this._animateTo(STATUS.SMALL);
        } else {
          this._animateTo(this.state.status);
        }
      },
  ...
  componentDidUpdate(prevProps: SwipeablePanelProps, prevState: SwipeablePanelState) {
    ...
    if (prevProps.openLarge !== openLarge) {
      this._animateTo(openLarge ? STATUS.LARGE : STATUS.SMALL)
    }
  ...
  _animateTo = (newStatus = 0) => {
    ...
    if (newStatus === STATUS.CLOSED) newY = PANEL_HEIGHT;
      ...
      newY = this.state.orientation === 'portrait' ? FULL_HEIGHT - 145 : FULL_HEIGHT / 3;
    else if (newStatus === STATUS.LARGE) newY = FULL_HEIGHT / 10;
    ...
    Animated.spring(this.state.pan, {
      ...
      if (newStatus === 1 && this.props.onSmall) {
        this.props.onSmall();
      }
      if (newStatus === 2 && this.props.onLarge) {
        this.props.onLarge();
      }
      if (newStatus === 0) {
        this.props.onClose();
  ...
  render() {
    ...
    return showComponent ? (
      ...
      >
        ...
        <View
          style={SwipeablePanelStyles.logo}>
          <Image
            source={require('../../../assets/bird_transparent.png')}
            style={SwipeablePanelStyles.logoImage}
          />
          <Text style={SwipeablePanelStyles.logoText}>Checkie</Text>
        </View>
        ...
        <ScrollView
          ...
          onContentSizeChange={ (width, height) => setTimeout(() => this.refs.panelContainer ? this.refs.panelContainer.scrollTo({y: height}) : null, 0) }
